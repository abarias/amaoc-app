...
{
    [MobileAppController]
    public class AMAOCNotificationsController : ApiController
    {
        public async Task<HttpResponseMessage> Post(string pns, string password, [FromBody]string message)
        {
            ...
            switch (pns.ToLower())
            {
                case "wns":
                    ...
                    outcome = await AMAOCNotifications.Instance.Hub.SendWindowsNativeNotificationAsync(toast);
                    break;
                case "apns":
                    // iOS
                    var alert = "{\"aps\":{\"alert\":\"" + message + "\"}}";
                    outcome = await AMAOCNotifications.Instance.Hub.SendAppleNativeNotificationAsync(alert);
                    break;
                case "gcm":
                    // Android
                    var notif = "{ \"data\" : {\"message\":\"" + message + "\"}}";
                    outcome = await AMAOCNotifications.Instance.Hub.SendGcmNativeNotificationAsync(notif);
                    break;
            }
